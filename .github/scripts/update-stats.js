...
// a Sunday 51 weeks before the current week: 52 week columns including
// the current partial week, always less than a year even on Saturdays.
function dateRange(now = new Date()) {
  ...
  start.setUTCDate(start.getUTCDate() - 51 * 7);
...
async function graphQL(query, variables) {
  ...
  // Keep credentials out of shell commands and child-process error output.
...
function getLatestCommitTimestamp() {
  ...
  return require("child_process")
    .execFileSync("git", ["log", "-1", "--format=%cI", "HEAD", "--", ".", ":(exclude)data/stats.json", ":(exclude)sitemap.xml"], {
      encoding: "utf8",
    })
    .trim();
}
...
async function main() {
  if (!LOGIN || !REPO) throw new Error("GH_LOGIN and GH_REPO env vars are required.");
...
function readPreviousStats(outPath) {
  ...
  try {
    const stats = JSON.parse(fs.readFileSync(outPath, "utf8"));
...
// Keep lastmod tied to a content change, not the scheduled stats refresh.
function bumpSitemapLastmod(updatedAt) {
  const sitemapPath = path.join(process.cwd(), "sitemap.xml");
  if (!fs.existsSync(sitemapPath)) {
    console.warn("sitemap.xml not found; skipping lastmod bump.");
    return;
  }
  const today = new Date(updatedAt).toISOString().slice(0, 10); // YYYY-MM-DD
